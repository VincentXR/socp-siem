...
<template>
  <div class="page-pad view-enter">
    ...
    <PageHeader :title="t('refset.title')" :description="t('refset.description')">
      <template #actions><el-button v-if="canWrite" type="primary" size="small" @click="dialogVisible = true">{{ t('refset.createSet') }}</el-button></template>
    </PageHeader>
    ...
    <el-drawer :model-value="Boolean(selectedId)" :title="selected?.name || t('forms.entries')" size="min(760px, 96vw)" @update:model-value="value => { if (!value) selectedId = '' }">
      <template v-if="selected">
        ...
        <div v-if="canWrite" class="section-toolbar"><el-input v-model="entryText[selected.id]" :placeholder="t('refset.addItem')" @keyup.enter="addEntry(selected.id)" /><el-button :loading="actionBusy" @click="addEntry(selected.id)">{{ t('common.add') }}</el-button><el-button @click="importing = true">{{ t('forms.import') }}</el-button></div>
