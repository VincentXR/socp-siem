...
function openTechniqueEdit(technique: Technique) {
  ...
  techniqueForm.value = {
    name: technique.name, tactic: technique.tactic, url: technique.url, description: technique.description,
  }
...
async function saveTechnique() {
  ...
  try {
    ...
    ElMessage.success('ATT&CK 技术已更新')
    await loadAttack()
  } catch (error) {
    ElMessage.error(error instanceof Error ? error.message : '更新 ATT&CK 技术失败')
  }
}
  ...
  <div class="page-pad view-enter">
    ...
    <el-card shadow="never">
      <el-table :data="techniques" size="small" border>
        ...
        <el-table-column label="操作" width="125"><template #default="{ row }"><el-button link type="primary" size="small" @click="openUrl(row.url)">详情</el-button><el-button link type="primary" size="small" @click="openTechniqueEdit(row as Technique)">编辑</el-button></template></el-table-column>
      </el-table>
    </el-card>

    <el-dialog v-model="techniqueDialogVisible" :title="`编辑 ATT&CK 技术 · ${editingTechniqueId}`" width="620px">
      <el-form label-width="80px">
        <el-form-item label="名称" required><el-input v-model="techniqueForm.name" /></el-form-item>
        <el-form-item label="战术"><el-select v-model="techniqueForm.tactic" style="width: 240px"><el-option v-for="tactic in tactics" :key="tactic.id" :label="tactic.name" :value="tactic.id" /></el-select></el-form-item>
        <el-form-item label="详情链接"><el-input v-model="techniqueForm.url" /></el-form-item>
        <el-form-item label="描述"><el-input v-model="techniqueForm.description" type="textarea" :rows="4" /></el-form-item>
      </el-form>
      <template #footer><el-button @click="techniqueDialogVisible = false">取消</el-button><el-button type="primary" @click="saveTechnique">保存</el-button></template>
    </el-dialog>
  </div>
</template>
