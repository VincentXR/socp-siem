...
function openCreateIoc(): void { editingIocId.value = null; resetIocForm(); showIocDialog.value = true }
...
async function importIocFile(event: Event): Promise<void> {
  ...
  try {
    ...
    const imported = await threatIntelApi.bulkImport(payload)
