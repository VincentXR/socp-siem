...
async function removeRule(id: string) {
  if (!confirm(locale.value === 'zh-CN' ? '确认删除该规则？删除后立即热更新引擎。' : 'Delete rule? Engine will hot reload.')) return
...
<template>
  <div class="page-pad view-enter">
    ...
    <el-row :gutter="12" style="margin-bottom:14px">
      <el-col :span="6"><el-card shadow="never"><div class="stat-card"><div class="num">{{ gasStat.rules ?? 0 }}</div><div class="label">{{ locale === 'zh-CN' ? '规则数' : 'Total Rules' }}</div></div></el-card></el-col>
      <el-col :span="6"><el-card shadow="never"><div class="stat-card"><div class="num">{{ gasStat.eventCount ?? 0 }}</div><div class="label">{{ locale === 'zh-CN' ? '事件数' : 'Events' }}</div></div></el-card></el-col>
      <el-col :span="6"><el-card shadow="never"><div class="stat-card"><div class="num" style="color:#f56c6c">{{ gasStat.alertCount ?? 0 }}</div><div class="label">{{ locale === 'zh-CN' ? '告警数' : 'Alarms' }}</div></div></el-card></el-col>
    ...
    <el-card shadow="never" style="margin-bottom:14px"><template #header>{{ locale === 'zh-CN' ? '模拟事件投递' : 'Simulate Event Ingestion' }}</template><div style="display:flex;gap:10px;align-items:center"><el-select v-model="ingestSource" style="width:120px"><el-option label="auth" value="auth" /><el-option label="web" value="web" /><el-option label="firewall" value="firewall" /></el-select><el-input v-model="ingestMsg" :placeholder="locale === 'zh-CN' ? '如：Failed password for admin' : 'e.g. Failed password for admin'" style="width:360px" /><el-button type="primary" @click="doIngest">{{ locale === 'zh-CN' ? '投递' : 'Ingest' }}</el-button><span v-if="ingestResult" class="mono" style="font-size:12px;color:#67c23a">{{ ingestResult }}</span></div></el-card>
    <el-card shadow="never"><template #header><div style="display:flex;justify-content:space-between;align-items:center"><span>{{ t('detect.rules') }}</span><el-button type="primary" size="small" @click="openRuleEditor()">{{ t('detect.createRule') }}</el-button></div></template>
      <el-table :data="rules" size="small" border><el-table-column prop="id" label="ID" width="150" show-overflow-tooltip /><el-table-column prop="name" :label="t('common.name')" min-width="150" show-overflow-tooltip /><el-table-column prop="type" :label="t('common.type')" width="90" /><el-table-column prop="severity" :label="t('common.severity')" width="85"><template #default="{ row }"><SevBadge :value="row.severity" /></template></el-table-column><el-table-column :label="locale === 'zh-CN' ? '匹配条件' : 'Conditions'" min-width="220" show-overflow-tooltip><template #default="{ row }">{{ (row.match || []).map((condition: { field: string; op: string; value: string }) => `${condition.field} ${condition.op} ${condition.value}`).join(' AND ') || (row.steps || []).length + ' steps' || '-' }}</template></el-table-column><el-table-column :label="t('common.enable')" width="70"><template #default="{ row }"><el-tag :type="row.enabled ? 'success' : 'info'" size="small">{{ row.enabled ? t('common.enabled') : t('common.disabled') }}</el-tag></template></el-table-column><el-table-column :label="t('common.actions')" width="170"><template #default="{ row }"><el-button link type="primary" size="small" @click="openRuleEditorRow(row)">{{ t('common.edit') }}</el-button><el-button link size="small" @click="toggleRuleRow(row)">{{ row.enabled ? t('common.disable') : t('common.enable') }}</el-button><el-button link type="danger" size="small" @click="removeRuleRow(row)">{{ t('common.delete') }}</el-button></template></el-table-column></el-table>
    </el-card>
    <el-dialog v-model="showRuleEditor" :title="ruleEditingId ? (locale === 'zh-CN' ? '编辑规则' : 'Edit Rule') : t('detect.createRule')" width="640px"><el-form label-width="90px"><el-form-item :label="t('common.name')"><el-input v-model="ruleForm.name" :placeholder="locale === 'zh-CN' ? '如：SSH 暴力破解' : 'e.g. SSH Brute Force'" /></el-form-item><el-form-item :label="t('common.type')"><el-radio-group v-model="ruleForm.type"><el-radio value="pattern">{{ locale === 'zh-CN' ? '模式' : 'Pattern' }}</el-radio><el-radio value="threshold">{{ locale === 'zh-CN' ? '阈值' : 'Threshold' }}</el-radio><el-radio value="correlation">{{ locale === 'zh-CN' ? '关联' : 'Correlation' }}</el-radio></el-radio-group></el-form-item><el-form-item :label="t('common.severity')"><el-select v-model="ruleForm.severity" style="width:160px"><el-option v-for="severity in SEVERITIES" :key="severity" :label="t('severities.' + severity) || severity" :value="severity" /></el-select></el-form-item><el-form-item :label="locale === 'zh-CN' ? '告警消息' : 'Alert Message'"><el-input v-model="ruleForm.message" :placeholder="locale === 'zh-CN' ? '支持 {key} {count} {host} 占位' : 'Supports {key} {count} {host}'" /></el-form-item><el-form-item :label="locale === 'zh-CN' ? '窗口' : 'Window'"><el-input v-model="ruleForm.window" :placeholder="locale === 'zh-CN' ? '如 60s / 5m / 1h' : 'e.g. 60s / 5m / 1h'" style="width:120px" /></el-form-item><template v-if="ruleForm.type === 'threshold'"><el-form-item :label="locale === 'zh-CN' ? '分组字段' : 'Key Field'"><el-select v-model="ruleForm.keyField" style="width:160px"><el-option v-for="field in COND_FIELDS" :key="field" :label="field" :value="field" /></el-select></el-form-item><el-form-item :label="t('detect.threshold')"><el-input v-model.number="ruleForm.threshold" type="number" style="width:120px" /></el-form-item></template><template v-if="ruleForm.type === 'correlation'"><el-form-item :label="locale === 'zh-CN' ? '关联字段' : 'Correlation Key'"><el-select v-model="ruleForm.keyField" style="width:160px"><el-option v-for="field in COND_FIELDS" :key="field" :label="field" :value="field" /></el-select></el-form-item><el-form-item :label="locale === 'zh-CN' ? '关联步骤' : 'Steps'"><div v-for="(step, stepIndex) in ruleForm.steps" :key="stepIndex" style="border:1px solid #e4e7ed;border-radius:6px;padding:8px;margin-bottom:8px"><div style="font-size:12px;color:#909399;margin-bottom:4px">Step {{ stepIndex + 1 }}</div><div v-for="(condition, conditionIndex) in step" :key="conditionIndex" style="display:flex;gap:6px;margin-bottom:4px"><el-select v-model="condition.field" size="small" style="width:110px"><el-option v-for="field in COND_FIELDS" :key="field" :label="field" :value="field" /></el-select><el-select v-model="condition.op" size="small" style="width:100px"><el-option v-for="op in COND_OPS" :key="op" :label="op" :value="op" /></el-select><el-input v-model="condition.value" size="small" placeholder="value" style="flex:1" /><el-button size="small" type="danger" link @click="step.splice(conditionIndex, 1)">x</el-button></div><el-button size="small" link type="primary" @click="ruleForm.steps[stepIndex].push({ field: 'msg', op: 'contains', value: '' })">+ Condition</el-button><el-button v-if="ruleForm.steps.length > 1" size="small" link type="danger" @click="ruleForm.steps.splice(stepIndex, 1)">Delete Step</el-button></div><el-button size="small" type="primary" plain @click="ruleForm.steps.push([{ field: 'msg', op: 'contains', value: '' }])">+ Step</el-button></el-form-item></template><el-form-item v-else :label="locale === 'zh-CN' ? '匹配条件' : 'Conditions'"><div v-for="(condition, conditionIndex) in ruleForm.match" :key="conditionIndex" style="display:flex;gap:6px;margin-bottom:4px;width:100%"><el-select v-model="condition.field" size="small" style="width:110px"><el-option v-for="field in COND_FIELDS" :key="field" :label="field" :value="field" /></el-select><el-select v-model="condition.op" size="small" style="width:110px"><el-option v-for="op in COND_OPS" :key="op" :label="op" :value="op" /></el-select><el-input v-model="condition.value" size="small" :placeholder="locale === 'zh-CN' ? '值（条件间为 AND）' : 'Value (AND)'" style="flex:1" /><el-button size="small" type="danger" link @click="ruleForm.match.splice(conditionIndex, 1)">x</el-button></div><el-button size="small" type="primary" plain @click="ruleForm.match.push({ field: 'msg', op: 'contains', value: '' })">+ Condition</el-button></el-form-item><el-form-item :label="t('common.enable')"><el-switch v-model="ruleForm.enabled" /></el-form-item></el-form><template #footer><el-button @click="showRuleEditor = false">{{ t('common.cancel') }}</el-button><el-button type="primary" @click="saveRule">{{ t('common.save') }}</el-button></template></el-dialog>
  </div>
</template>
