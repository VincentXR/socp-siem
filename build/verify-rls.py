#!/usr/bin/env python3
"""Assert every tenant-owned table has a PostgreSQL RLS backstop or a reviewed exemption.

Layer 3 of the tenant isolation contract (``infra/postgres/tenant-rls.sql``) is applied
outside Flyway, so nothing at build time proved that a table carrying ``tenant_id`` was
actually covered. A service could therefore ship an entity whose isolation existed only
as an application-layer predicate, and the loss of depth was invisible until someone
compared ``pg_class`` by hand. This gate replays the DDL of every Flyway location, maps
the tenant-owned JPA entities onto that replay, and requires a reviewed decision for
every table in the database surface:
