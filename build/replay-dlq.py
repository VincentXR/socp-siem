#!/usr/bin/env python3
"""Replay (redrive) records sitting in a SOCP ``<topic>-dlq`` back to its main topic.

The Kafka DLQ has been a one-way door: five dead-letter topics are written by the
consumers but nothing in the repository reads them back, and ``docs/idempotency-
contract.md`` itself concedes that a terminal ``DEAD`` row "只能由 DLQ/人工处理"
without shipping the tool for that handling. This closes that gap with an
*idempotent* re-drive.
