#!/usr/bin/env python3
"""Static gate for observability alert -> runbook wiring.

The holistic review found 11 alerts whose ``annotations`` carried only
summary/description, with ``runbook_url`` zero hits repo-wide — a fired page handed
the operator a name and no procedure. This gate keeps that from regressing without
depending on Helm:
...
* the event-path dashboard is valid JSON wired to the provisioned datasource uid.
...
def main() -> int:
    ...
    if errors:
        ...
        return 1
    print("Observability alert/runbook wiring passed: every alert has a resolvable runbook_url; "
