...
Guarantees the operational tooling the holistic review called missing actually
exists, stays registered against its runbook, and that the single machine-readable
database roster cannot silently fall behind the real per-service databases:
...
* ``build/postgres-databases.txt`` equals the ``CREATE DATABASE`` set in
  ...
  quietly under-scope);
* ``restore-postgres.sh`` re-applies tenant RLS and asserts it (the isolation
  recovery the review flagged as silent);
