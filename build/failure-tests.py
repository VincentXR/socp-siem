...
def main():
    ...
    if pb:
        st2, run_payload = api("/soar-web/api/runs", "POST", {
        ...
        check("Temporal 停时 durable Run 保持排队", st2 == 200 and ex.get("executionId"), f"st={st2} status={status}")
    else:
        check("Temporal 停时 durable Run 保持排队", False, "无已发布剧本")
