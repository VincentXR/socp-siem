...
# Each service still copies its own cross-cutting security/management block into
# application.yml (git 1be0858a paid for one added metrics-token key in 17 files).
# Until those blocks converge onto a starter-provided classpath fragment, the copies
# must stay identical key-for-key, so compare them here instead of trusting review.
SECURITY_CORE_KEYS = {
...
# detect-web keeps an in-flight routing refactor in its working tree; comparing its
# copy while that edit is uncommitted would make this gate flap for an unrelated
# reason. Re-enable by removing the entry once the refactor lands.
CONFIG_PARITY_EXCLUSIONS = {"detect-web": "in-flight cross-dimension routing refactor"}
...
def config_parity_checks(errors: list[str], modules: list[str]) -> int:
    ...
    for module in modules:
        if module in CONFIG_PARITY_EXCLUSIONS:
            continue
...
def main() -> int:
    ...
    if CONFIG_PARITY_EXCLUSIONS:
        print("config parity exclusions (not silent skips): "
              + "; ".join(f"{name}: {reason}" for name, reason in sorted(CONFIG_PARITY_EXCLUSIONS.items())),
              file=sys.stderr)
