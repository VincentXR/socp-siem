 ...
 */
@Component
public class SoarApprovalExpiryWorker {
    private final SoarApprovalRepository approvals;
    private final SoarService soar;

    public SoarApprovalExpiryWorker(SoarApprovalRepository approvals, SoarService soar) {
        this.approvals = approvals;
        this.soar = soar;
    }

    @Scheduled(fixedDelayString = "${socp.soar.approval-expiry-poll-ms:30000}",
            initialDelayString = "${socp.soar.approval-expiry-initial-delay-ms:30000}")
    @TenantSystemJob
    public void tick() {
