...
class SoarControllerTest {
    ...
    void createPlaybookReturns201ForAdminAndRejectsViewer() throws Exception {
        ...
        mvc.perform(post("/api/playbooks")
                        .header(HttpHeaders.AUTHORIZATION, BEARER)
    ...
    void getPlaybookAndPatchPlaybookWork() throws Exception {
        ...
        mvc.perform(get("/api/playbooks/pb-1")
                        .header(HttpHeaders.AUTHORIZATION, BEARER)
                        .header("X-Role", ROLE_ADMIN))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.id").value("pb-1"));

        mvc.perform(patch("/api/playbooks/pb-1")
                        .header(HttpHeaders.AUTHORIZATION, BEARER)
                        .header("X-Role", ROLE_ADMIN)
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void playbookVersionsWorkflowOperations() throws Exception {
        ...
        given(service.validateVersion("pb-1", 1))
                .willReturn(new DefinitionValidationResult(true, List.of(), List.of(), "soar.playbook", "hash1", 5, 2, 0));
        ...
        mvc.perform(get("/api/playbooks/pb-1/versions")
                        .header(HttpHeaders.AUTHORIZATION, BEARER)
                        .header("X-Role", ROLE_ADMIN))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data[0].version").value(1));

        mvc.perform(put("/api/playbooks/pb-1/versions/1")
                        .header(HttpHeaders.AUTHORIZATION, BEARER)
                        .header("X-Role", ROLE_ADMIN)
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        mvc.perform(post("/api/playbooks/pb-1/versions/1/validate")
                        .header(HttpHeaders.AUTHORIZATION, BEARER)
                        .header("X-Role", ROLE_ADMIN))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.valid").value(true));

        mvc.perform(post("/api/playbooks/pb-1/versions/1/publish")
                        .header(HttpHeaders.AUTHORIZATION, BEARER)
                        .header("X-Role", ROLE_ADMIN))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.status").value("PUBLISHED"));

        mvc.perform(post("/api/playbooks/pb-1/versions/1/deprecate")
                        .header(HttpHeaders.AUTHORIZATION, BEARER)
                        .header("X-Role", ROLE_ADMIN))
                .andExpect(status().isOk())
    ...
    void runDetailAndLifecycleOperations() throws Exception {
        ...
        mvc.perform(get("/api/runs/run-100")
                        .header(HttpHeaders.AUTHORIZATION, BEARER)
                        .header("X-Role", ROLE_ADMIN))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.runId").value("run-100"));

        mvc.perform(post("/api/runs/run-100/cancel")
                        .header(HttpHeaders.AUTHORIZATION, BEARER)
                        .header("X-Role", ROLE_ADMIN)
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        mvc.perform(post("/api/runs/run-100/retry")
                        .header(HttpHeaders.AUTHORIZATION, BEARER)
                        .header("X-Role", ROLE_ADMIN)
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        mvc.perform(post("/api/runs/run-100/rerun")
                        .header(HttpHeaders.AUTHORIZATION, BEARER)
                        .header("X-Role", ROLE_ADMIN)
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void resolveUnknownActionOutcome() throws Exception {
        ...
        mvc.perform(post("/api/node-runs/node-1/resolve-unknown")
                        .header(HttpHeaders.AUTHORIZATION, BEARER)
                        .header("X-Role", ROLE_ADMIN)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(json.writeValueAsString(req)))
    ...
    void approvalsAndDecisions() throws Exception {
        ...
        mvc.perform(post("/api/approvals/appr-1/decisions")
                        .header(HttpHeaders.AUTHORIZATION, BEARER)
                        .header("X-Role", ROLE_ADMIN)
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    @Test
    void legacyAlertPayloadOnEventRouteIsConvertedToTypedEnvelope() throws Exception {
        given(automationRules.evaluate(anyMap()))
                .willReturn(Map.of("eventId", "AL-1", "matchedRuns", 0));
        String path = "/api/events/evaluate";
        ...
        verify(automationRules).evaluate(argThat(event ->
                "soar.event".equals(event.get("schemaVersion"))
    ...
    void manualTasksCompletion() throws Exception {
        ...
        mvc.perform(get("/api/manual-tasks")
                        .header(HttpHeaders.AUTHORIZATION, BEARER)
                        .header("X-Role", ROLE_ADMIN))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data[0].id").value("task-1"));

        mvc.perform(post("/api/manual-tasks/task-1/complete")
                        .header(HttpHeaders.AUTHORIZATION, BEARER)
                        .header("X-Role", ROLE_ADMIN)
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void templatesCatalogAndInstall() throws Exception {
        ...
        mvc.perform(post("/api/templates/high-risk-ioc/install")
                        .header(HttpHeaders.AUTHORIZATION, BEARER)
                        .header("X-Role", ROLE_ADMIN))
                .andExpect(status().isCreated())
    ...
    void automationRulesEndpoints() throws Exception {
        ...
        mvc.perform(get("/api/automation-rules?page=0&size=20")
                        .header(HttpHeaders.AUTHORIZATION, BEARER)
                        .header("X-Role", ROLE_ADMIN))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.items[0].id").value("rule-1"));

        mvc.perform(post("/api/automation-rules/test")
                        .header(HttpHeaders.AUTHORIZATION, BEARER)
                        .header("X-Role", ROLE_ADMIN)
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        mvc.perform(post("/api/automation-rules/rule-1/enable")
                        .header(HttpHeaders.AUTHORIZATION, BEARER)
                        .header("X-Role", ROLE_ADMIN))
                .andExpect(status().isOk())
    ...
    void connectorsAndOperations() throws Exception {
        ...
        given(service.definitionSchema()).willReturn(json.createObjectNode().put("title", "soar.playbook"));
        ...
        mvc.perform(post("/api/connections/conn-1/test")
                        .header(HttpHeaders.AUTHORIZATION, BEARER)
                        .header("X-Role", ROLE_ADMIN))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.status").value("HEALTHY"));

        mvc.perform(get("/api/operations/dead-dispatches")
                        .header(HttpHeaders.AUTHORIZATION, BEARER)
                        .header("X-Role", ROLE_ADMIN))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data[0].id").value("dead-1"));

        mvc.perform(post("/api/operations/dead-dispatches/dead-1/requeue")
                        .header(HttpHeaders.AUTHORIZATION, BEARER)
                        .header("X-Role", ROLE_ADMIN)
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        mvc.perform(get("/api/definition-schema")
                        .header(HttpHeaders.AUTHORIZATION, BEARER)
                        .header("X-Role", ROLE_ADMIN))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.title").value("soar.playbook"));
    }
}
