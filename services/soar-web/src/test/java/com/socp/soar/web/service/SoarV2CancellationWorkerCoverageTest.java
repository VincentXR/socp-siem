...
class SoarV2CancellationWorkerCoverageTest {
    ...
    private SoarV2CancellationWorker worker;

    @BeforeEach
    void setUp() {
        TenantContext.set("tenant-a");
        worker = new SoarV2CancellationWorker(runs, temporal);
    }
    ...
    void cancellingRunIsSignalledAndProjectionTouched() {
        SoarRunEntity run = run("CANCELLING", "soar-v2-tenant-a-run-1");
    ...
    void runWithoutWorkflowIdIsSkipped() {
        ...
        verify(temporal, never()).cancelV2(anyString());
    ...
    void signalFailureKeepsRunCancellingForTheNextTick() {
        ...
        willThrow(new IllegalStateException("temporal unreachable")).given(temporal)
                .cancelV2("soar-v2-tenant-a-run-1");
