...
class SoarV2RunRecoveryWorkerCoverageTest {
    ...
    private SoarV2RunRecoveryWorker worker;

    @BeforeEach
    void setUp() {
        TenantContext.set("tenant-a");
        worker = new SoarV2RunRecoveryWorker(runs, temporal, STALE_SECONDS);
    }
    ...
    void closedWorkflowTerminalizesProjectionAsActionUnknown() {
        SoarRunEntity run = staleRun("RUNNING", "soar-v2-tenant-a-run-1");
        given(runs.findTop100ByStatusInAndUpdatedAtBeforeOrderByUpdatedAtAsc(anyCollection(), any()))
                .willReturn(List.of(run));
        given(temporal.describeV2("soar-v2-tenant-a-run-1")).willReturn(TemporalExecutor.V2WorkflowState.CLOSED);
    ...
    void openWorkflowKeepsProjectionUntouched() {
        SoarRunEntity run = staleRun("RUNNING", "soar-v2-tenant-a-run-2");
        given(runs.findTop100ByStatusInAndUpdatedAtBeforeOrderByUpdatedAtAsc(anyCollection(), any()))
                .willReturn(List.of(run));
        given(temporal.describeV2("soar-v2-tenant-a-run-2")).willReturn(TemporalExecutor.V2WorkflowState.OPEN);
    ...
    void unknownDescribeKeepsProjectionUntouched() {
        SoarRunEntity run = staleRun("DISPATCHING", "soar-v2-tenant-a-run-3");
        given(runs.findTop100ByStatusInAndUpdatedAtBeforeOrderByUpdatedAtAsc(anyCollection(), any()))
                .willReturn(List.of(run));
        given(temporal.describeV2("soar-v2-tenant-a-run-3")).willReturn(TemporalExecutor.V2WorkflowState.UNKNOWN);
    ...
    void rowTerminalizedAfterTheSnapshotIsSkipped() {
        SoarRunEntity completed = staleRun("SUCCEEDED", "soar-v2-tenant-a-run-4");
        SoarRunEntity refreshed = staleRun("RUNNING", "soar-v2-tenant-a-run-5");
    ...
    void runWithoutUpdatedAtIsSkipped() {
        SoarRunEntity run = staleRun("RUNNING", "soar-v2-tenant-a-run-6");
    ...
    void workerWithoutTemporalClientMarksEveryStaleRunTimedOut() {
        SoarV2RunRecoveryWorker noTemporal = new SoarV2RunRecoveryWorker(runs);
        SoarRunEntity run = staleRun("RUNNING", "soar-v2-tenant-a-run-7");
