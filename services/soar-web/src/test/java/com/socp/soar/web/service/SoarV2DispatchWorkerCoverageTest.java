...
class SoarV2DispatchWorkerCoverageTest {
    ...
    private SoarV2DispatchWorker worker;

    @BeforeEach
    void setUp() {
        TenantContext.set("tenant-a");
        worker = new SoarV2DispatchWorker(dispatches, runs, versions, temporal, new ObjectMapper());
    }
    ...
    void claimedPendingRowStartsTemporalWithStableWorkflowId() {
        ...
        ArgumentCaptor<SoarV2WorkflowRequest> request = ArgumentCaptor.forClass(SoarV2WorkflowRequest.class);
        ArgumentCaptor<String> workflowId = ArgumentCaptor.forClass(String.class);
        verify(temporal).startV2(request.capture(), workflowId.capture());
        assertThat(workflowId.getValue()).isEqualTo("soar-v2-tenant-a-run-1");
        ...
        assertThat(run.getTemporalWorkflowId()).isEqualTo("soar-v2-tenant-a-run-1");
        assertThat(run.getTemporalRunId()).isEqualTo("temporal-run-1");
        assertThat(outbox.getStatus()).isEqualTo("DISPATCHED");
        assertThat(outbox.getClaimedBy()).startsWith("soar-v2-");
