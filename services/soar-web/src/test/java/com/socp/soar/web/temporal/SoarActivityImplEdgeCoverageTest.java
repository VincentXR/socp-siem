...
/** Edge coverage for the SOAR Activity: degraded registry, corrupt JSON, lock fallbacks. */
@ExtendWith(MockitoExtension.class)
class SoarActivityImplEdgeCoverageTest {
    ...
    void compensateNodeWithoutRegistryReportsUnavailable() {
        SoarActivityImpl degraded = new SoarActivityImpl(executor, runs, nodeRuns, events,
                approvals, attempts, connectors, null, secretResolver, manualTasks, mapper);
        SoarNodeResult result = degraded.compensateNode(request("endpoint/isolate-host"),
    ...
    void executeNodeToleratesCorruptInputAndConnectionJsonAndUnlockedAttemptLookup() {
        ...
        SoarNodeResult result = activity.executeNode(
                new SoarNodeRequest(TENANT, RUN_ID, NODE_ID, "ACTION", "http.webhook/request",
    ...
    void markRunWaitingFallsBackToUnlockedApprovalLookupAndSanitizesCorruptSnapshot() {
        ...
        activity.markRunWaitingWithContext(TENANT, RUN_ID, NODE_ID, 3600, 1,
    ...
    private static SoarNodeRequest request(String actionRef) {
        return new SoarNodeRequest(TENANT, RUN_ID, NODE_ID, "ACTION", actionRef, "", "{}",
                "idem-1", "", Map.of(), 0);
    }
