...
class SoarSignalWorkerCoverageTest {
    ...
    private SoarSignalWorker worker;

    @BeforeEach
    void setUp() {
        TenantContext.set("tenant-a");
        worker = new SoarSignalWorker(signals, runs, temporal, new ObjectMapper());
    }
    ...
    void approvalSignalWithGateKeyIsDeliveredThroughTheGateDecision() {
        ...
        givenPending(signal, run("WAITING_APPROVAL", "soar-tenant-a-run-1"));
    ...
    void expiredApprovalSignalUsesTheExpireGatePath() {
        ...
        verify(temporal).decideGate("soar-tenant-a-run-1", false, "gate-2", true);
        assertThat(signal.getStatus()).isEqualTo("SENT");
    }
    ...
    void legacyApprovalSignalWithoutGateKeyUsesPlainDecision() {
        ...
        verify(temporal).decide("soar-tenant-a-run-1", true);
        assertThat(signal.getStatus()).isEqualTo("SENT");
    }
    ...
    void manualTaskSignalWithNodeIdTargetsTheNode() {
        ...
        verify(temporal).completeManualTaskForNode("soar-tenant-a-run-1", "node-1", "{\"ticket\":\"T-1\"}");
        assertThat(signal.getStatus()).isEqualTo("SENT");
    }
    ...
    void manualTaskSignalWithoutNodeIdUsesTheLegacyEntryPoint() {
        ...
        verify(temporal).completeManualTask("soar-tenant-a-run-1", "{}");
        assertThat(signal.getStatus()).isEqualTo("SENT");
    }
    ...
    void lateSignalForTerminalRunIsCancelledWithoutTouchingTemporal() {
        ...
        givenPending(signal, run("PARTIALLY_SUCCEEDED", "soar-tenant-a-run-1"));
    ...
    void signalForCancellingRunIsCancelledWithoutTouchingTemporal() {
        ...
        givenPending(signal, run("CANCELLING", "soar-tenant-a-run-1"));
    ...
    void deliveryFailureRequeuesWithBackoff() {
        ...
        doThrow(new IllegalStateException("temporal signal failed"))
                .when(temporal).decideGate(anyString(), anyBoolean(), anyString(), anyBoolean());
