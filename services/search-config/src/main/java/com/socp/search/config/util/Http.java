package com.socp.search.config.util;
...
/** 极简同步 HTTP 客户端（best-effort，用于采集管线向 DETECT 转发归一化事件）。 */
public final class Http {
    ...
    /** POST 并读取响应体（成功返回响应字符串，失败返回 null）。用于批量转发后读 DETECT 的 accepted/rejected。 */
    public static String postBody(String url, String body, String contentType, int timeoutMs) {
        try {
            HttpURLConnection c = (HttpURLConnection) new URL(url).openConnection();
            c.setRequestMethod("POST");
            c.setConnectTimeout(timeoutMs);
