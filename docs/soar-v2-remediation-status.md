...
| 5 | P1 Workbench 没有 SOAR V2 浏览器流程 | **基础主流程已覆盖，完整流程仍待补齐** | `frontend/apps/workbench/e2e/soar-v2.spec.ts` 已覆盖创建草稿、编辑/保存、校验/发布、运行检查器、版本回显、审批和人工任务完成；同时修复了深链初始化和编辑器初始 dirty 状态。 | 仍需执行、恢复、SSE 断线、权限负例和截图/可访问性验收；当前测试使用确定性 API fixture，不能替代目标环境验收。 |
