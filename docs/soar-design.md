...
`CANCELLING` 是取消意图，不是远程结束证明。取消信号重试不会更新运行的业务进度时间。
停滞恢复先在事务外查询 Temporal，再在短事务中核对 run 版本；仅明确关闭或可信的
工作流缺失响应可用于终态恢复，未知响应保留当前状态。两类维护任务用独立的数据库
调度时间领取和轮转，不覆盖活动进度。具体判据、截止时间与 V25 升级要求见
[运行手册的“取消与停滞恢复”](soar-runbook.md)。
