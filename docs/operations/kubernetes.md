...
**Why `SET NOT NULL` is the sharp edge here.** The repository has no `DROP COLUMN`,
...
The gate that would enforce "a release may not pair a `SET NOT NULL`/drop with a
same-release rollback" is **not** in place: `verify-migrations.py` would need a
`compat-window` marker on shrinking DDL and a widened destructive pattern
(`DROP COLUMN|RENAME|DROP NOT NULL|SET NOT NULL|ADD COLUMN NOT NULL`). That expansion,
and moving migration execution out of the boot process into a Helm `pre-upgrade` hook
Job (so "all migrations succeed before any Deployment rolls"), are tracked as deferred:
the Job needs a migrate-and-exit entrypoint the service images do not yet have, and both
touch `build/verify-migrations.py` and the migration executor model that are outside this
change's ownership. Until then, follow the expand-then-contract discipline above by hand.