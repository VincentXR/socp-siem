...
The reproducible EPS benchmark was removed as unreproducible (commit `e4c1e43d`),
and `docs/validation-matrix.md` no longer has a runnable "Scale baseline". That was
the honest call: a fabricated number is worse than none. What replaced it must not be
a hand-ticked boolean either, so this page gives the **derivation procedure**: the
bounded ceilings the code already enforces, and the exact queries on metrics the
event path already exports that turn observed load into a capacity statement.
...
1. **Capacity is deployment-owned evidence.** Do not promote a figure here to an
   ...
   [production-readiness.md](../production-readiness.md), ADR 007, and
   [maturity-matrix.md](../maturity-matrix.md) the repository does not certify EPS,
...
| Detection engine queue | fixed `ArrayBlockingQueue(100_000)` per engine | `platform/socp-rule/.../RuleEngine.java` | effective replicas <= partitions; queue saturation surfaces as lag, not a dropped event (bounded, fail-closed) |
| Search fallback buffer | per-tenant cap (count-bounded) | `services/search-config/.../SearchStore.java` | degraded-buffer depth; buffer is count-bounded (a known open item — byte bound not yet enforced) |
