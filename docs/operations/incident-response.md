...
Without rule enablement and Alertmanager routing, the default posture produces
no pages. Verify the complete path with a synthetic test alert before relying on
...
matching detections are lost while other pipeline metrics can remain healthy.
Lag-based `absent()` cannot detect this rule-level failure.
**Action**:
...
3. If the rule is broken, disable it explicitly and open a detection-content ticket.
...
The Compose baseline does not mount the assets under `infra/observability`.
An environment that enables them must add the following read-only mounts and
Prometheus rule registration:
