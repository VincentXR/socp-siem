...
Five dead-letter topics are written by the consumers (each derives `topic + "-dlq"`
in code — see [Kafka topic provisioning](kafka-topic-provisioning.md)) but the
platform treats a dead letter as terminal: it never reads them back. The
[idempotency contract](../idempotency-contract.md) states plainly that a terminal
`DEAD` "只能由 DLQ/人工处理", and this runbook is the "人工处理" half of that
promise: how to decide *why* a record died, whether it is safe to re-drive, and
how to confirm the pipeline is healthy again afterwards.
...
The five dead-letter topics are **not** homogeneous. Only one writer produces a
re-drivable envelope; the others store the raw value with a thin or missing reason
(the review of the DLQ surface confirmed 4 of 5 lack a usable reason):
...
* **Detect-path topics (`socp-events`, `socp-detection-routed-v2`) without `--force`.**
  ...
  handled the journal row (see the deferred endpoint note below).
...
## Detection-path terminal rows (the gap this runbook cannot close alone)
...
A first-class reset (`POST` to detect-web admin that clears a terminal journal row and
re-emits) is the proper fix and is tracked as deferred: it lives in `detect-web` engine
code that is mid-refactor on this branch, so it is intentionally not wired here.
