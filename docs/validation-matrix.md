...
| Cross-cutting slice | `python build/verify-slice.py` | Authentication, the current second-tenant write ID and its scoped readback, audit, rate limiting, and trace propagation | PR/release candidate |
| Event pipeline | `python build/verify-pipeline.py` | One generated event ID/tenant/content in Kafka and OpenSearch, its Detection alarm in Alert and ClickHouse, and an available non-degraded ClickHouse report | Middleware change/scheduled |
