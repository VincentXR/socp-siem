...
SOCP enforces tenant boundaries at three layers:
...
void rebuildAllTenantIndexes() {
    // Explicit system scope for cross-tenant maintenance.
}
```
...
the policy after all service schemas exist. Seed and data migrations run with
the migration role or an explicit `SET socp.tenant_id='*'`.
